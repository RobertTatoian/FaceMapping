...
public class DetectedFace {
	...
	*/
	 ...
	 *
	 * @param oldPixel The first pixel value
	 * @param newPixel The second pixel value
	 * @param weight   The weight of the second pixel value, should be in [0, 1]
	...
	private int blendPixels(int oldPixel, int newPixel, float weight)
		{
			...
			int r = (int)((((oldPixel & 0xFF0000) >> 16) * (1 - weight))
			        + (((newPixel & 0xFF0000) >> 16) * weight));
			int g = (int)((((oldPixel & 0x00FF00) >> 8) * (1 - weight))
	 ...
	 *
	 * @param m A matrix of BGR pixels.
	 ...
	 * texture. The new texture is given more weight than the preexisting texture.
	 *
	 * The two textures should have the same dimensions
	 *
	 * @param oldTexture The preexisting texture
	 * @param newFace    A texture to be blended into the old texture.
	 */
	 ...
	 *
	 * @param face     An OpenCV matrix that contains a frontal face
	 * @param leftEye  The coordinates of the left eye, {x, y}
	 * @param rightEye The coordinates of the right eye, {x, y}
	 */
	public void updateFrontTexture(Mat face, float[ ] leftEye, float[ ] rightEye)
		{
			...
			double[ ] newRightEye = new double[ ] {
			        ...
			        ((rightEye[0] - center.x) * Math.sin(angle)) + ((rightEye[1] - center.y) * Math.cos(angle))
			                + center.y };
			Point eyeCenter = new Point((newLeftEye[0] + newRightEye[0]) / 2,
	 ...
	 *
	 * @param profile		 An OpenCV matrix containing a side face.
	 * @param eyeCoordinates The coordinates of the eye in the profile, {x, y}
	 * @param facingRight    True if the face is facing right, false otherwise
	 */
