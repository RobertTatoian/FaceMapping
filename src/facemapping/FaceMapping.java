...
public class FaceMapping extends PApplet {
	...
	String					face_cascade_name	= "C:\\OpenCV-3.0.0\\sources\\data\\haarcascades\\haarcascade_frontalface_alt.xml";
	String					eyes_cascade_name	= "C:\\OpenCV-3.0.0\\sources\\data\\haarcascades\\haarcascade_eye_tree_eyeglasses.xml";
	...
	public void settings( ) {
		...
		println("Is the camera open: " + camera.isOpened());

		cv_width = camera.get(Videoio.CV_CAP_PROP_FRAME_WIDTH);
		cv_height = camera.get(Videoio.CV_CAP_PROP_FRAME_HEIGHT);

		println("The width of the camera being used is: " + (int) cv_width);
		println("The height of the camera being used is: " + (int) cv_height);
	...
	public void setup( ) {
		...
		if (!eyes_cascade.load(eyes_cascade_name))
			{
				println("Error loading eyes_cascade_name.xml");
			} else
			{
				println("Succeded loading eyes_cascade_name.xml");
			}
	...
	public void draw( ) {
		...
		img = new PImage(toBufferedImage(frame)); // Convert the frame with the
													 // detected face to a
													 // PImage
	 ...
	 * by a PImage to work with processing.
	 * 
	 * Found at:
