	...
	public boolean intersects(Cube cube)
		{
			...
			for (PVector pos : vertices)
				{
					if (absoluteBoundingBox.isInside(pos.x, pos.y, pos.z))
						{
							return true;
						}
				}
	...
	public boolean intersectsSAT(Cube cube)
		{
			...
			for (PVector normal : normalVecs)
				{
					if (intersectsAlongAxis(myVertices, cubeVertices, normal))
						return true;
				}
			...
			for (PVector myNormal : normalVecs)
				for (PVector cubeNormal : cubeNormals)
					{
						if (intersectsAlongAxis(myVertices, cubeVertices, myNormal.cross(cubeNormal)))
							return true;
					}
	...
	private boolean intersectsAlongAxis(List<PVector> myVertices, List<PVector> otherVertices, PVector axis)
		{
			...
			// Find the max and min coordinate of each cube along the axis by projecting
	...
	private void computeAbsoluteBoundingBox()
		{
			...
			absoluteBoundingBox =  new BoundingBox3D(xMin, yMin, zMin,
